...
class ArtworkCategory extends Component {
    constructor(props) {
        super(props);
        this._flatList = null;
            ...
            sectionContent: [
                {
                    ...
                    data: [{
                        name: '张三',
                        coverUrl: 'https://facebook.github.io/react-native/docs/assets/favicon.png',
                        id: '1'
                    }]
                },
                {
    ...
    componentDidMount() {
        ...
        // 添加监听者
        this.listener = DeviceEventEmitter.addListener('searchArtwork', (text) => {
            this.props.navigation.navigate('ArtworkList', {searchContent: text});
        });
    ...
    async fetchCategory() {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                await this.fetchArtworksBySecondType();
            } else {
                API.toastLong('操作失败')
            }
        } catch (error) {
    ...
    async fetchArtworksBySecondType() {
        let newSectionData = [];
        for (let i = 0; i < this.state.CateData[this.state.selectedRootCate].secondTypeItems.length; i++) {
            ...
            try {
                ...
                let response = await API._fetch(API.GET({url: '/user/allArtwork/type', formData}));
                let responseJson = await response.json();
                if (response.status === 200) {
                    newSectionData.push({
                        key: this.state.CateData[this.state.selectedRootCate].secondTypeItems[i].value,
                        data: responseJson.artworkItemDtos
                    })
                } else {
                    API.toastLong('操作失败');
                    return
        ...
        return (
            <TouchableOpacity
                ...
                onPress={async () => {
                    ...
                    this.setState({selectedRootCate: index}, () => this.fetchArtworksBySecondType());
    ...
    renderCell(item, index) {
        return (
            <TouchableOpacity
                ...
                onPress={() => {
                    this.props.navigation.navigate('ArtworkDetail', {id: item.id});
                }}
...
export default connect(state => state.reducer)(ArtworkCategory)